...
审查日期：2026-09-13。目标是判断 MoonKeyguard 是否只是 MoonBVHKit、近期参赛项目或已有 Mooncakes 包的改名包装。
...
未发现一个同时提供“快捷键 DSL + 父子 context 重叠 + chord 前缀 + 平台保留键 + 冲突图 + SARIF + 迁移门禁”的 MoonBit 项目。MoonKeyguard 也不是对上述项目的移植：生产代码、结构体、规则和 81 个测试均为本仓库重新实现。
